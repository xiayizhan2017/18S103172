...
public class Filter {
	...
	public static List<Tweet> writtenBy(List<Tweet> tweets, String username) {
		...
		for (Tweet it : tweets)
			if (it.getAuthor().equals(username))
				result.add(it);
		return result;
	}
	...
	public static List<Tweet> inTimespan(List<Tweet> tweets, Timespan timespan) {
		List<Tweet> result = new ArrayList<Tweet>();
		for (Tweet it : tweets) {
			if (it.getTimestamp().isAfter(timespan.getStart()) && it.getTimestamp().isBefore(timespan.getEnd()))
				result.add(it);
		}
		return result;
	}
	...
	public static List<Tweet> containing(List<Tweet> tweets, List<String> words) {
		...
		for (Tweet it : tweets) {
			for (String s : words)
				if (it.getText().toLowerCase().indexOf(s.toLowerCase()) > -1)
					result.add(it);
		}
		return result;
	}
